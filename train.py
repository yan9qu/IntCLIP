...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(test_dataset, batch_size=cfg.DATALOADER.VAL.BATCH_SIZE,
    ...
    if cfg.TRAINER.FINETUNE_BACKBONE:
        ...
        base_group = {'params': backbone_params,
    ...
    if cfg.TRAINER.FINETUNE_LAST_LAYER:
        try:
            ...
            intent_params = model.module.intent_params()
        print('num of params in intent layer: ', len(intent_params))
        intent_group = {'params': intent_params,
                        'lr': cfg.OPTIM.LR * cfg.OPTIM.ATTN_LR_MULT}
        sgd_polices.append(intent_group)

    if cfg.TRAINER.FINETUNE_LAST_LAYER:
        try:
            aggregation_params = model.aggregation_params()
        except:
            aggregation_params = model.module.aggregation_params()
        print('num of params in intent layer: ', len(aggregation_params))
        aggregation_params = {'params': aggregation_params,
                              'lr': cfg.OPTIM.LR * cfg.OPTIM.ATTN_LR_MULT}
        sgd_polices.append(aggregation_params)
    ...
    if args.resume is not None:
        if os.path.exists(args.resume):
            ...
            print('... loading pretrained weights from %s' %
                  args.resume, file=logfile, flush=True)
    ...
    for epoch in range(args.start_epoch, cfg.OPTIM.MAX_EPOCH):
        batch_time, losses, mAP_batches = train_coop(
        ...
        if (epoch + 1) % args.val_every_n_epochs == 0 or epoch == args.stop_epochs - 1:
            p_c, r_c, f_c, p_o, r_o, f_o, mAP_score = validate(
                val_loader, model, args)
            avg_scores = p_c
            is_best = avg_scores > best_avg_scores
            if is_best:
                best_mAP = mAP_score
    ...
    print('... loading pretrained weights from %s' %
          best_checkpoints, flush=True)
    print('... loading pretrained weights from %s' %
          best_checkpoints, file=logfile, flush=True)
