...
from utils.helper import AverageMeter, mAP, calc_F1, compute_f1
import time
import torch
import sys
sys.path.insert(0, '../')
...
def validate(data_loader, model, args):
    ...
    with torch.no_grad():
        ...
        # f1_all = all_f1_calc(torch.cat(targets, dim=0).cpu().numpy(), torch.cat(preds, dim=0).cpu().numpy())
        # print('F1_all: ', f1_all)
        all_targets = torch.cat(targets, dim=0).cpu().numpy()
        all_preds = torch.cat(preds, dim=0).cpu().numpy()
        mAP_score = mAP(all_targets, all_preds)
        f1 = {}
        f1["micro"], f1["samples"], f1["macro"], f1["none"] = compute_f1(
            all_targets, all_preds)
        # print micro, samples and macro F1. keep two decimals
        print('F1_macro: {:2.2f} \t F1_micro: {:2.2f} \t F1_samples: {:2.2f} \t mAP: {:2.2f}'.format(
            f1["macro"]*100, f1["micro"]*100, f1["samples"]*100,  mAP_score))

        avg_scores = f1["macro"]*100 + f1["micro"] * \
            100 + f1["samples"]*100 + mAP_score
        avg_scores /= 4

    torch.cuda.empty_cache()
    return avg_scores, mean_r_c, mean_f_c, p_o, r_o, f_o, mAP_score
