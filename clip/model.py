...
class AttentionConv(nn.Module):
    def __init__(self, embed_dim: int, spec_dim: int,  output_dim: int = None):
        ...
        self.positional_embedding = nn.Parameter(torch.randn(
            spec_dim ** 2 + 1, embed_dim) / embed_dim ** 0.5)
        self.k_proj = nn.Linear(embed_dim, embed_dim)
        self.q_proj = nn.Linear(embed_dim, embed_dim)
        self.v_proj = nn.Linear(embed_dim, embed_dim)
        self.c_proj = nn.Linear(embed_dim, output_dim or embed_dim)
    ...
    def forward(self, x):
        ...
        if h * w + 1 != self.positional_embedding.shape[0]:
            w_spacial = self.positional_embedding[1:].permute(
                1, 0).reshape(1, -1, self.spec_dim, self.spec_dim)
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        self.attnpool = AttentionPool2d(
            input_resolution // 32, self.embed_dim, heads, output_dim)
...
class PAA(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        # set gamma as learnable parameter, initialize as 0.1
        # self.gamma = 0.01
        self.gamma = nn.Parameter(torch.zeros(1))

        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x_obj, x_sub):
        """
            inputs :
        ...
        """
        m_batchsize, C, height, width = x_obj.size()
        proj_query = self.query_conv(x_obj).view(
            m_batchsize, -1, width*height).permute(0, 2, 1)
        proj_key = self.key_conv(x_obj).view(m_batchsize, -1, width*height)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
        proj_value = self.value_conv(x_sub).view(m_batchsize, -1, width*height)

        out = torch.bmm(proj_value, attention.permute(0, 2, 1))
        out = out.view(m_batchsize, C, height, width)

        out = self.gamma*out + (x_sub + x_obj)/2
        return out
...
class ModifiedResNet_conv_proj(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        self.position_aware_aggregation = PAA(width*8)
        ...
        # residual layers for learning part
        self._inplanes = 512

        self.intent_layer3 = self._make_layer(width * 4, layers[2], stride=2)
        self.intent_layer4 = self._make_layer(width * 8, layers[3], stride=2)

        self.embed_dim = width * 32  # the ResNet feature dimension
        self.attnpool = AttentionConv(
            self.embed_dim, input_resolution // 32, output_dim)

        self.intent_atnpool = AttentionConv(
            self.embed_dim, input_resolution // 32, output_dim)

        self.merge_self_attention = nn.MultiheadAttention(
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        intent_x = self.intent_layer3(intent_x)
        intent_x = self.intent_layer4(intent_x)

        # x = x + intent_x

        x, attn_weights = self.attnpool(x)
        # intent_x, _ = self.attnpool(intent_x)
        intent_x, _ = self.intent_atnpool(intent_x)
        # fuse the intent_x and x by a self-attention module
        # remove the position embedding
        x = x[:, :, 1:]
        intent_x = intent_x[:, :, 1:]
        pos_emb_intent_x = intent_x[:, :, 0]
        # reshape to [batch_size, embed_dim, h, w] ,h=w=7
        x = x.reshape(x.shape[0], x.shape[1], 7, 7)
        intent_x = intent_x.reshape(intent_x.shape[0], intent_x.shape[1], 7, 7)

        # x = x + intent_x
        intent_x = self.position_aware_aggregation(intent_x, x)

        # reshape
        intent_x = intent_x.reshape(intent_x.shape[0], intent_x.shape[1], -1)

        # add the position embedding back
        intent_x = torch.cat([pos_emb_intent_x.unsqueeze(2), intent_x], dim=2)

        return intent_x, attn_weights
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spacial = state_dict[k][1:].permute(
                    1, 0).reshape(1, -1, 7, 7)
                new_size = int((old_state_dict[k].shape[0] - 1) ** 0.5)
                w_spacial = F.interpolate(w_spacial, size=(
                    new_size, new_size), mode='bicubic')
            ...
            # load the same weights for intent layers
            if 'intent' in k and 'visual' in k and 'atnpool' not in k:
                old_state_dict[k] = old_state_dict[k.replace('intent_', '')]
                print(f'load {k} from {k.replace("intent_", "")}')
            # load the same weights for intent attn pooling
            if 'intent_atnpool' in k and 'visual' in k:
                old_state_dict[k] = old_state_dict[k.replace(
                    'intent_atnpool', 'attnpool')]
                print(
                    f'load {k} from {k.replace("intent_atnpool", "attnpool")}')
